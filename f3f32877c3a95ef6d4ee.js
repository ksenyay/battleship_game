...
class ComputerShipPlacer {
  ...
  generateShip(shipLength) {
    while (true) {
      ...
      if (shipCoordinates.length === shipLength && this.isValidPlacement(shipCoordinates)) {
...
class ManageDOM {
  ...
  makeComputerMove() {
    setTimeout(() => {
      ...
      let cell = document.querySelector(`.grid-element-Player[data-col="${y}"][data-row="${x}"]`);
      ...
      if (hitOrMiss === "Hit!") {
        ...
        coordinates.forEach(coord => {
          let gridElements = document.querySelectorAll(`.grid-element-Player[data-col="${coord.y}"][data-row="${coord.x}"]`);
  ...
  addTargetCells(x, y) {
    ...
    possibleMoves.forEach(move => {
      ...
      if (move.x >= 0 && move.x < 10 && move.y >= 0 && move.y < 10 && !this.computerAttacks.has(key)) {
      ...
      cell.addEventListener("drop", event => {
        ...
        if (this.isComputer === false) {
          if (areShipsPlaced && this.currentPlayer === this.player1) {
            ...
            document.querySelectorAll(`.grid-element-${this.player1.name}`).forEach(item => {
  ...
  makeMove() {
    ...
    document.querySelectorAll(".grid-container div").forEach(item => {
      ...
      item.addEventListener("click", event => {
        let opponent;
        if (this.isComputer === false) {
          opponent = this.currentPlayer === this.player1 ? this.player2 : this.player1;
        } else {
